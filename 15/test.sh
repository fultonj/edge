...
if [[ -z $1 ]]; then
    ...
    export AZ=$1
    if [[ ! -d $AZ ]]; then
        echo "USAGE: $0 [AZ] (where AZ matches a directory containing deployment data)"
        echo "Eligible directories found for AZ are: "
        ls -Fd edge* | grep -v common | grep /
        exit 1
    fi
fi
...
if [[ $DISC -eq 1 ]]; then
    ...
    if [[ ! -e central/config-download/inventory.yaml ]]; then
        mkdir central/config-download/
        tripleo-ansible-inventory --static-yaml-inventory central/config-download/inventory.yaml --stack central
    fi
...
if [[ $CEPH -eq 1 ]]; then
    if [[ ! $AZ/config-download/inventory.yaml ]]; then
        mkdir $AZ/config-download/
        tripleo-ansible-inventory --static-yaml-inventory central/config-download/inventory.yaml --stack $AZ
    fi
    INV="$AZ/config-download/inventory.yaml --limit mons[0]"
    SUFFIX=$(ls $AZ/config-download/host_vars/ | head -1)
